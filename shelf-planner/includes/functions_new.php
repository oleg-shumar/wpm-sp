...
";
	...
	$forecast_data   = QAMain_Core::parse_forecast_file( SP_ROOT_DIR . '/forecast.json' );
	...
	foreach ( $products_data as &$products_item ) {
		...
		if ( $is_min_stock_based ) {
			...
			if ( isset( $products_item['weeks_of_stock_product'] ) && is_numeric( $products_item['weeks_of_stock_product'] ) && $products_item['weeks_of_stock_product'] > 0 ) {
				$weeks_of_stock = $products_item['weeks_of_stock_product'];
			}
	...
	foreach ( $orders_data as &$order_record ) {
		...
		if ( $order_record['variation_id'] ) {
			$order_record['product_id'] = $order_record['variation_id'];
		}
	}
